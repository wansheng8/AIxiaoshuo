...
import { TEARDOWN_TABS } from "../domain/pipeline";
import type { Teardown } from "../domain/types";

export const TEAR_TABS = TEARDOWN_TABS;

export type TearTabId = string;
...
export function skillFilled(row: Teardown, skillId: string) {
  ...
  if (!tab) return false;
  return Boolean(String((row as unknown as Record<string, unknown>)[tab.field] || "").trim());
}

...
**本文导航**：[系统要求](#系统要求) · [一、先选一条路线](#一先选一条路线) · [二、准备：先认识三样东西](#二准备先认识三样东西) · [三、安装必备工具](#三安装必备工具) · [四、下载代码（所有路线第一步）](#四下载代码所有路线第一步) · [五、路线 A：本机直接部署](#五路线-a本机直接部署个人电脑推荐新手) · [六、路线 B：Docker Compose](#六路线-bdocker-compose服务器推荐) · [七、配置模型](#七配置模型必须做否则无法生成) · [八、第一次使用：写出第一章](#八第一次使用写出第一章) · [九、局域网与公网访问](#九局域网与公网访问) · [十、日常维护](#十日常维护) · [十一、进阶](#十一进阶) · [十二、排错大全](#十二排错大全)
...
不确定就选路线 A。三条路线都要先完成下面的第二、三、四步。想先在本机快速体验，也可以直接用 `bash scripts/setup.sh` 加 `bash scripts/serve.sh start` 两条命令完成路线 A。

...
function lastBeat(text) {
  const t = String(text || "")
    .replace(/\s+/g, " ")
    .trim();
  if (!t) return "";
  const parts = t.split(/[。！？!?]+/).map((s) => s.trim()).filter((s) => s.length > 4);
  const beat = parts.pop() || t;
  return beat.slice(-48);
}

function scoreMood(text, cues) {
  let n = 0;
...
function emotionBridgeLines(text, kind) {
  const guess = inferEmotion(text);
  const lines = ["# 情绪衔接"];
  if (guess) {
    const where = kind === "continue" ? "已有正文" : "上一章";
    lines.push(`${where}落在「${guess.mood}」。末句还停在：${guess.beat}`);
    lines.push(`从这里的人物身体、没说完的话和物件温度接着长。开场贴着这个落点，让情绪自己变。`);
  } else if (kind === "continue") {
    lines.push(`贴着已有正文最后的人物状态、没说完的话和场面温度接着写。`);
  } else {
    lines.push(`上一章已经把情绪写进场面。本章从上一章结尾的人物身体、没说完的话和物件温度接着长。`);
  }
  return lines;
}

module.exports = { inferEmotion, lastBeat, emotionBridgeLines };
